    ...
    tg.showPopup({
        ...
        if (btnId === 'buy') {
            ...
            if (!currentUser.subscriptions.autoBoost) {
                currentUser.subscriptions.autoBoost = {active: true, carIds: [], expiresAt: null};
            }
            ...
            currentUser.subscriptions.autoBoost.expiresAt = expiresAt.toISOString();
            
            DB.saveUser(currentUser);
            renderMyListings();
            renderProfile();
            
            tg.showAlert(`Автоподнятие активировано!\nОбъявление будет подниматься 5 раз в день.\nАктивно до: ${expiresAt.toLocaleDateString()}`);
        }
    });
}
...
function manageAutoBoost(carId) {
    ...
    const carTitle = `${car.brand} ${car.model} ${car.year || ''}`.trim();
    const expiresAt = new Date(currentUser.subscriptions.autoBoost.expiresAt);
    
    tg.showPopup({
        title: 'Автоподнятие',
        message: `${carTitle}\n\nАвтоподнятие активно\nДо: ${expiresAt.toLocaleDateString()}\n\nПоднимается 5 раз в день:\n00:00, 05:00, 10:00, 15:00, 19:00`,
        buttons: [
...
function disableAutoBoost(carId) {
    ...
    // Если больше нет объявлений с автоподнятием - деактивируем подписку
...
function renderMyListings() {
    ...
    container.innerHTML = myListings.map(car => {
        ...
        // Кнопка автоподнятия
        if (hasAutoBoost) {
            autoBoostButton = `<button class="my-listing-autoboost active" onclick="event.stopPropagation(); manageAutoBoost(${car.id})">🔄 Автоподнятие активно</button>`;
        } else {
