// ============================================
// AUTOMARKET v5 - ФИНАЛЬНАЯ ВЕРСИЯ
// ============================================
...
// Кнопки "Все >"
function showAllTop() {
    currentSection = 'all-top';
    render();
}

function showAllNew() {
    currentSection = 'all-new';
    render();
}
...
// РЕНДЕРИНГ - Топ/Новинки скролл, Все - grid
function render() {
    ...
    if (currentSection === 'all-top') {
        // Показать все топ
        topContainer.style.display = 'none';
        newContainer.style.display = 'none';
        allContainer.className = 'listings-grid';
        allContainer.style.display = 'grid';
        allTitle.style.display = 'block';
        allContainer.innerHTML = top.map(makeCard).join('');
    } else if (currentSection === 'all-new') {
        // Показать все новые
        topContainer.style.display = 'none';
        newContainer.style.display = 'none';
        allContainer.className = 'listings-grid';
        allContainer.style.display = 'grid';
        allTitle.style.display = 'block';
        allContainer.innerHTML = newCars.map(makeCard).join('');
    } else {
        // Обычный режим - скролл для топ/новинок
...
function showDetail(id) {
    ...
    // ИСПРАВЛЕНО: Используем USERNAME для Telegram
    if (sellerUsername) {
...
// ИСПРАВЛЕНО: Telegram через username
...
function handleSubmit(e) {
    ...
    console.log('Объявление добавлено:', newCar);
    console.log('Всего объявлений:', cars.length);
    ...
    if (tgUser) {
        ...
        if (!users[userId]) {
            users[userId] = {
                id: userId,
                telegramId: userId,
                username: tgUser.username || '',  // ВАЖНО: сохраняем username
