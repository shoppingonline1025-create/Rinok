...
async function syncFromFirebase() {
    ...
    try {
        ...
        if (!data) {
            ...
            const needsEnrich = fbCars.some(c => !c.sellerInfo);
            if (needsEnrich) {
...
// Дополнительная защита: запрещаем резиновый скролл на iOS
document.addEventListener('touchmove', function(e) {
    // Разрешаем скролл только внутри скроллируемых элементов
    let el = e.target;
    let canScroll = false;
    while (el && el !== document.body) {
        ...
        if (overflow.includes('scroll') || overflow.includes('auto')) {
            // Разрешаем если элемент реально можно прокрутить
...
function closePage(id) {
    document.getElementById(id).classList.remove('show');
    // iOS: возвращаем скролл body
    document.body.style.overflow = '';
    document.body.style.position = '';
}

function openPageWithLock(id) {
    // iOS: фиксируем body чтобы не скроллился под открытой страницей
    document.getElementById(id).classList.add('show');
