...
function updateFormBrandOptions() {
    ...
    // Заполняем опции partType и condition если они пустые
    if (isParts && typeof PARTS_TYPES !== 'undefined') {
        const partTypeEl = document.getElementById('partType');
        const conditionEl = document.getElementById('condition');
        
        if (partTypeEl && partTypeEl.options.length <= 1) {
            PARTS_TYPES.forEach(type => {
                const opt = document.createElement('option');
                opt.value = type;
                opt.textContent = type;
                partTypeEl.appendChild(opt);
            });
        }
