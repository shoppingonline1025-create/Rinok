...
function openTopUp() {
    tg.showPopup({
        ...
        message: 'Функция пополнения будет доступна позже.\n\nДля тестирования нажмите ОК — будет добавлено 100 лей.',
        buttons: [
            {id: 'add100', type: 'default', text: 'Добавить 100 лей'},
            {id: 'cancel', type: 'cancel', text: 'Отмена'}
        ]
    }, (buttonId) => {
        if (buttonId === 'add100') {
            addBalance(100, 'test');
            tg.showAlert('Баланс пополнен на 100 лей!');
        }
    });
}
...
// ─── Поднятие объявлений ──────────────────────────────────────
function canFreeBoost(car) {
    if (!car.freeBoostAvailableAt) return true; // первое поднятие
    const now = new Date();
    const available = new Date(car.freeBoostAvailableAt);
    return now >= available;
}

function getNextFreeBoostTime(car) {
    if (!car.freeBoostAvailableAt) return null;
    const now = new Date();
    const available = new Date(car.freeBoostAvailableAt);
...
function boostListing(carId, paid = false) {
    ...
    if (paid || !isFree) {
        ...
        // Бесплатное поднятие
        performBoost(car);
        // Следующее бесплатное поднятие завтра в 00:00
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        tomorrow.setHours(0, 0, 0, 0);
        car.freeBoostAvailableAt = tomorrow.toISOString();
...
function openAutoBoostSubscription() {
    const myListings = cars.filter(c => c.userId === currentUser.id);
    
    if (myListings.length === 0) {
        tg.showAlert('У вас пока нет объявлений');
        return;
    }
    
    const isActive = currentUser.subscriptions?.autoBoost?.active;
    const expiresAt = currentUser.subscriptions?.autoBoost?.expiresAt;
    const activeCarIds = currentUser.subscriptions?.autoBoost?.carIds || [];
    
    let message = '📦 Подписка "Автоподнятие"\n\n';
    message += '✓ Автоматическое поднятие 5 раз в день\n';
    message += '  (00:00, 05:00, 10:00, 15:00, 19:00)\n';
    message += '✓ Бесплатное ручное поднятие остаётся\n\n';
    message += 'Стоимость: 50 лей/мес за 1 объявление\n';
    message += '+20 лей за каждое дополнительное\n\n';
    
    if (isActive) {
        const exp = new Date(expiresAt);
        message += `Активна до: ${exp.toLocaleDateString()}\n`;
        message += `Объявлений: ${activeCarIds.length}`;
    } else {
        message += `Ваш баланс: ${currentUser.balance} лей`;
    }
    
    const buttons = isActive ? [
        {id: 'manage', type: 'default', text: 'Управление'},
        {id: 'cancel', type: 'cancel', text: 'Закрыть'}
    ] : [
        {id: 'buy', type: 'default', text: 'Купить за 50 лей'},
        {id: 'cancel', type: 'cancel', text: 'Отмена'}
    ];
    
    tg.showPopup({title: 'Автоподнятие', message, buttons}, (btnId) => {
        if (btnId === 'buy') {
            selectCarsForAutoBoost();
        } else if (btnId === 'manage') {
            manageCarsForAutoBoost();
        }
    });
}

function selectCarsForAutoBoost() {
    // Открываем модальное окно выбора объявлений
    const myListings = cars.filter(c => c.userId === currentUser.id);
    // TODO: создать UI для выбора
    tg.showAlert('Функция в разработке. Пока доступно через команды.');
}

function manageCarsForAutoBoost() {
...
function renderMyListings() {
    ...
    container.innerHTML = myListings.map(car => {
        ...
        const isFree = canFreeBoost(car);
        const nextFree = getNextFreeBoostTime(car);
        const hasAutoBoost = currentUser.subscriptions?.autoBoost?.active && 
                            currentUser.subscriptions.autoBoost.carIds.includes(car.id);
        
        let boostButton = '';
        if (hasAutoBoost) {
            boostButton = '<div class="my-listing-autoboost">🔄 Автоподнятие активно</div>';
        } else if (isFree) {
