...
function showDetail(id) {
    ...
    let sellerPhone = seller?.phone || c.sellerInfo?.phone || '+373 XXX XX XXX';
    ...
    let sellerTelegramId = seller?.telegramId || c.sellerInfo?.telegramId || '';
    let sellerCity = seller?.city || c.sellerInfo?.city || '';
    let sellerRating = seller?.rating || c.sellerInfo?.rating || 0;
    // Кнопка Telegram: есть если username ИЛИ реальный telegramId (не тестовый user_X)
    const hasTelegramContact = sellerUsername || 
        (sellerTelegramId && !String(sellerTelegramId).startsWith('user_') && sellerTelegramId !== 'test_user');
    
    let contactButtons = '';
    
    if (hasTelegramContact) {
        contactButtons = `<button class="contact-btn telegram-btn" onclick="openTelegramChat('${sellerUsername}', '${sellerTelegramId}')">
...
function openTelegramChat(username, telegramId) {
    if (username && username !== 'testuser') {
        // Открываем по username (надёжнее)
        tg.openTelegramLink ? tg.openTelegramLink(`https://t.me/${username}`) 
                            : window.open(`https://t.me/${username}`, '_blank');
    } else if (telegramId && !String(telegramId).startsWith('user_') && telegramId !== 'test_user') {
        // Открываем по ID — работает в Telegram даже без username
        window.open(`tg://user?id=${telegramId}`, '_blank');
    } else {
        tg.showAlert('Контакт продавца недоступен');
    }
}
...
function handlePhotos(e) {
    ...
    // Если добавляем к уже загруженным - объединяем (до 6)
    const preview = document.getElementById('photoPreview');
    
    // Читаем файлы последовательно (важно для iOS)
    let loaded = 0;
    files.forEach((file, index) => {
        ...
        reader.onload = function(event) {
            ...
            div.innerHTML = `<img src="${event.target.result}"><button type="button" class="file-preview-remove" onclick="removePhoto(${idx})">×</button>`;
...
function renderProfileAvatar() {
    ...
    if (currentUser.photo) {
        wrap.style.backgroundImage = `url('${currentUser.photo}')`;
