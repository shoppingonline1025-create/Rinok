...
function toggleTopAll() {
    ...
    if (topExpanded) {
        ...
        // Рендерим все топ объявления
        let filtered = getFilteredCars();
        let top = filtered.filter(c => c.isTop);
        expanded.innerHTML = top.map(makeCard).join('');
    } else {
        btn.textContent = 'Все ›';
...
function toggleAllAll() {
    ...
    if (allExpanded) {
        ...
        // Рендерим все остальные объявления (не топ и не новые)
        let filtered = getFilteredCars();
        ...
        expanded.innerHTML = allCars.map(makeCard).join('');
    } else {
        btn.textContent = 'Все ›';
        scroll.style.display = 'flex';
        expanded.style.display = 'none';
...
function render() {
    ...
    if (topExpanded) {
        document.getElementById('topAllExpanded').innerHTML = top.map(makeCard).join('');
    }
    
    if (newExpanded) {
        document.getElementById('newAllExpanded').innerHTML = newCars.map(makeCard).join('');
    }
    
    if (allExpanded) {
        document.getElementById('allAllExpanded').innerHTML = allCars.map(makeCard).join('');
    }
}
    ...
    return `<div class="car-card" onclick="showDetail(${c.id})">
        ...
        <div class="car-info">
            ...
            <div class="car-details">
                <div>Пробег: ${fmt(c.mileage)} км</div>
                <div>Объем: ${c.engine}</div>
                <div>Топливо: ${c.fuel}</div>
                ${c.drive ? `<div>Привод: ${c.drive}</div>` : ''}
                <div>Город: ${c.city}</div>
                <div>Регистрация: ${c.registration}</div>
            </div>
        </div>
    </div>`;
}
...
function showDetail(id) {
    ...
    if (c.photos && c.photos.length > 0) {
        const images = c.photos.map((p, i) => 
            `<img src="${p}" style="display: ${i === 0 ? 'block' : 'none'}; width: 100%; object-fit: cover;">`
        ).join('');
...
function handleSubmit(e) {
    ...
    const carData = {
        ...
        drive: document.getElementById('drive')?.value || null,  // Добавлено поле привода
    ...
    if (editingCarId) {
        ...
        cars = DB.updateCar(editingCarId, carData);
