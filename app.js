// ============================================
// AUTOMARKET v6.1 - ИСПРАВЛЕНИЕ НАВИГАЦИИ
// Дата обновления: 2026-02-10 20:00
// ============================================

console.log('=== AUTOMARKET v6.1 ЗАГРУЖЕН ===');
...
let detailHistory = []; // Стек истории просмотренных объявлений
...
function showDetail(id) {
    console.log('=== ОТКРЫТИЕ ОБЪЯВЛЕНИЯ ===');
    console.log('ID объявления:', id);
    
    let c = cars.find(x => x.id === id);
    if (!c) {
        console.error('Объявление не найдено! ID:', id);
        console.log('Всего объявлений в массиве cars:', cars.length);
        console.log('Все ID:', cars.map(car => car.id));
        return;
    }
    
    console.log('Объявление найдено:', c.brand, c.model);
    console.log('UserID продавца:', c.userId);
    ...
    console.log('Имя продавца:', sellerName);
    console.log('Username продавца:', sellerUsername);
    ...
    console.log('➜ Добавлено в историю. Текущий стек:', detailHistory);
    console.log('➜ Размер стека истории:', detailHistory.length);
    
    document.getElementById('detailPage').classList.add('show');
}

// Навигация назад с историей
function goBackFromDetail() {
    console.log('╔════════════════════════════════════╗');
    console.log('║      КНОПКА НАЗАД (v6.1)          ║');
    console.log('╚════════════════════════════════════╝');
    console.log('Текущая история:', detailHistory);
    console.log('Длина истории:', detailHistory.length);
    
    // Убираем текущее объявление из истории
    if (detailHistory.length > 0) {
        const removed = detailHistory.pop();
        console.log('Убрали из истории ID:', removed);
    }
    
    // Если в истории есть предыдущие объявления
    if (detailHistory.length > 0) {
        const previousId = detailHistory[detailHistory.length - 1];
        console.log('➜ Возврат к объявлению ID:', previousId);
        
        // Убираем последний элемент, т.к. showDetail добавит его снова
        detailHistory.pop();
        
        // Показываем предыдущее объявление
        showDetail(previousId);
    } else {
        // История пуста - закрываем страницу
        console.log('➜ История пуста - закрываем страницу');
        detailHistory = [];
        closePage('detailPage');
    }
}

// Открытие объявления из ленты (сброс истории)
function openDetail(id) {
    console.log('╔════════════════════════════════════╗');
    console.log('║ ОТКРЫТИЕ НОВОГО ОБЪЯВЛЕНИЯ (v6.1) ║');
    console.log('╚════════════════════════════════════╝');
    console.log('ID:', id);
    console.log('Сброс истории навигации');
    detailHistory = []; // Сбрасываем историю
    showDetail(id);
}
...
function handleSubmit(e) {
    ...
    if (editingCarId) {
        ...
        console.log('Текущее количество объявлений:', cars.length);
        ...
        console.log('Новое объявление ID:', carData.id);
        console.log('Данные объявления:', carData);
        ...
        console.log('Количество объявлений после добавления:', cars.length);
        
        // Сохраняем в DB
        DB.saveCars(cars);
        
        // Проверяем что сохранилось
        const savedCars = DB.getCars();
        console.log('Количество объявлений в DB после сохранения:', savedCars.length);
        ...
        console.log('Объявление добавлено в список пользователя');
    ...
    // КРИТИЧЕСКИ ВАЖНО: обновляем глобальную переменную cars
    cars = DB.getCars();
    console.log('Финальное количество объявлений после перезагрузки:', cars.length);
...
function initTestUsers() {
    ...
    let users = DB.getUsers();
    console.log('Текущее количество пользователей в БД:', Object.keys(users).length);
    ...
    testUsers.forEach(u => {
        if (!users[u.id]) {
            ...
            created++;
            console.log(`Создан тестовый пользователь: ${u.id} (${u.firstName} ${u.lastName})`);
        } else {
            console.log(`Пользователь ${u.id} уже существует`);
        }
    });
    
    DB.saveUsers(users);
    console.log(`Тестовых пользователей создано: ${created}`);
    console.log('Всего пользователей в БД:', Object.keys(users).length);
    console.log('Список всех пользователей:', Object.keys(users));
}
...
function renderMyListings() {
    ...
    container.innerHTML = myListings.map(car => {
        ...
        if (car.photos && car.photos.length > 0) {
            thumbHtml = `<div class="my-listing-thumb" onclick="openDetail(${car.id})" style="background-image: url('${car.photos[0]}'); background-size: cover; background-position: center;"></div>`;
        } else {
            thumbHtml = `<div class="my-listing-thumb" onclick="openDetail(${car.id})">${emoji}</div>`;
        }
        
        return `<div class="my-listing-item">
            ${thumbHtml}
            <div class="my-listing-info" onclick="openDetail(${car.id})">
